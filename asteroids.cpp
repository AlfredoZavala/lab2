...
public:
	Game() {
		...
		// changing the color of the asteroids	
			a->color[0] = rnd();
		        a->color[1] = rnd();
		        a->color[2] = rnd();
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt rad = ((g.ship.angle+90.0) / degree360) * PI * 2.0;
					//convert angle to a vector
					Flt xdir = cos(rad);
					Flt ydir = sin(rad);
					// bullet position its shot from
					b->pos[0] += xdir*2.0f;
					b->pos[1] += ydir*2.0f;
					// speed of bullets 
	...
	if (e->type == MotionNotify) {
		if (savex != e->xbutton.x || savey != e->xbutton.y) {
			...
			if (xdiff > 0) {
				...
				if (g.ship.angle >= degree360)
					g.ship.angle -= degree360;
			}
			else if (xdiff < 0) {
				g.ship.angle += 0.05f * (float)xdiff;
			...
			if (ydiff > 0) {
				...
				//apply thrust
				//convert ship angle to radians
				Flt rad = ((g.ship.angle+degree90) / degree360) * PI * 2.0;
				//convert angle to a vector
				Flt xdir = cos(rad);
				Flt ydir = sin(rad);
...
void buildAsteroidFragment(Asteroid *ta, Asteroid *a)
{
	...
	// asteroids are the same color as their original one	
		...
		while (i < g.nbullets) {
			...
			if (dist < (a->radius*a->radius)) {
				...
				//this asteroid is hit.
			    	...
			    	// incrementing destroyed asteroids
				g.dasteroids++;	
